pub mod msg_parser {
    use mqtt_v5::{
        types::{ Packet, ConnectPacket, PublishPacket, QoS, ProtocolVersion }, encoder, decoder, 
    };
    ...
    pub fn cm_decode(buffer: &mut BytesMut) -> Result<mqtt_v5::types::Packet, String> {
        if buffer.is_empty() {
            ...
            Ok(decoder::decode_mqtt(buffer, ProtocolVersion::V500).unwrap().unwrap())
        }
    }
    ...
    pub fn test_publish_encode_decode() {
        ...
        match decode2 {
            Ok(Packet::Publish(p)) => 
                println!("\tPublish packet received {:?}",p.packet_id)
            ,
            _=> panic!("Incorrect type returned"),
        };
    }
